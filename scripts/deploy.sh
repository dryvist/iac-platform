...
# The Semaphore runner's shared self-registration token, generated by the
# ansible-proxmox-apps openbao role (openbao_generated_app_secrets.semaphore).
...
if [ "${1:-}" = "--inner" ]; then
  ...
  [ -n "${SEMAPHORE_RUNNER_REGISTRATION_TOKEN:-}" ] || { echo "SEMAPHORE_RUNNER_REGISTRATION_TOKEN missing from OpenBao $SEMAPHORE_BAO_PATH" >&2; exit 1; }
